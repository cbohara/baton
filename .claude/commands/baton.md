...
The heavy gates — browser QA, visual regression — are expensive and
...
## Leg 1 — Spec (baton-code-explorer → baton-spec-writer)
- If the argument is a GitHub issue number or URL, fetch it: `gh issue view <n>`.
- **First**, delegate to the **baton-code-explorer** subagent for a lightweight, read-only
  trace of the code the task actually touches. It returns a short code map — relevant
  files, key symbols, patterns to honor, likely change sites — not a full audit.
- **Then** delegate to the **baton-spec-writer** subagent, handing it that code map, to turn
  the input into a concrete spec with explicit, testable acceptance criteria and an
  explicit out-of-scope list. The map grounds the spec in the real system, so its
  boundaries and constraints reflect the code that's there, not the one imagined when
...
## Leg 2 — Red (baton-test-writer) (this is the most important gate)
- Delegate to the **baton-test-writer** subagent to write tests covering the acceptance
...
## Leg 3 — Green (baton-implementer)
- Delegate to the **baton-implementer** subagent to make the failing tests pass.
...
## Leg 4 — Verify the running app (baton-qa-browser) — if enabled / web UI
- If this is a web app and QA is enabled in CLAUDE.md, start the dev server (or use the
  preview URL), then delegate to the **baton-qa-browser** subagent.
...
- Any FAIL is a blocker → back to the **baton-implementer** (leg 3). Keep the report for the PR.

## Leg 5 — Review (baton-reviewer) (loop until clean)
- Delegate to the **baton-reviewer** subagent (fresh, adversarial, read-only).
- It weighs the QA report as evidence alongside its own read.
- If it raises blocking issues, hand them back to the **baton-implementer**, then re-run the
...
- Print a short summary: criteria met, unit + full-suite status,
